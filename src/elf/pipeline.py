...
from src.elf.native_encoder import T5_HIDDEN_DIM, ELFNativeEncoder
...
class ELFPipeline:
    ...
    def enhance(
        ...
        if (
            ...
            return self._enhance_with_denoiser(text, steps, noise_t, cfg_scale, rng, blend_alpha)
    ...
    def _enhance_with_denoiser(
        ...
        if self._denoiser is None:
            raise RuntimeError("denoiser 不可用")
        if not isinstance(self.encoder, ELFNativeEncoder):
            raise RuntimeError("denoiser 增强需要 ELFNativeEncoder")

        pooled = self.encoder.encode_pooled(text)  # (512,) raw hidden
        z0 = np.asarray(pooled, dtype=np.float32) / LATENT_STD  # latent 归一化
        if rng is None:
            rng = np.random.default_rng()
        eps = rng.standard_normal(T5_HIDDEN_DIM, dtype=np.float32) * DENOISER_NOISE_SCALE
        z_t = ((1.0 - noise_t) * z0 + noise_t * eps).astype(np.float32)

        # ODE 去噪: t 从 noise_t 线性降至 t_eps, 网络预测 x0, v = (x0 - z)/(1-t)
        t_vals = np.linspace(noise_t, T_EPS, steps + 1)
        ...
        pooled_out = np.asarray(z0_pred, dtype=np.float32) * LATENT_STD  # denormalize
    ...
    def _denoiser_step(
        ...
        支持单条 (512,) 和批量 (N, 512) 输入, 自动检测 ndim。
        """
        if self._denoiser is None:
            raise RuntimeError("denoiser 不可用")

        z_arr = np.asarray(z, dtype=np.float32)
        batch_mode = z_arr.ndim == 2
        B = z_arr.shape[0] if batch_mode else 1
        dev = self._denoiser.device

        z_t = torch.from_numpy(z_arr).view(B, 1, -1).to(dev)
        t_t = torch.tensor([t], device=dev)
        scale_t = torch.tensor([1.0], device=dev)
        if x_pred_prev is None:
            x_prev = torch.zeros_like(z_t)
        else:
            xp_arr = np.asarray(x_pred_prev, dtype=np.float32)
            x_prev = torch.from_numpy(xp_arr).view(B, 1, -1).to(dev)
        x_den = max(1.0 - t, T_EPS)

        # 条件预测
        net = self._denoiser(torch.cat([z_t, x_prev], dim=-1), t_t, self_cond_cfg_scale=scale_t)
        x_cond = net[:, 0].detach().cpu().numpy().astype(np.float32)  # (B, 512)
        if not batch_mode:
            x_cond = x_cond.reshape(-1)
        v_cond = ((x_cond - z_arr) / x_den).astype(np.float32)

        if abs(cfg_scale - 1.0) < 1e-6:
            return v_cond, x_cond

        # 无条件预测: latent 清零(z=0, self-cond=0)
        z_u = torch.zeros(B, 1, self._denoiser.text_encoder_dim, device=dev)
        net_u = self._denoiser(torch.cat([z_u, z_u], dim=-1), t_t, self_cond_cfg_scale=scale_t)
        x_uncond = net_u[:, 0].detach().cpu().numpy().astype(np.float32)  # (B, 512)
        if not batch_mode:
            x_uncond = x_uncond.reshape(-1)
    ...
    def enhance_batch(
        ...
        当加载了 ELF-B denoiser(issue #37)且未注入自定义速度场时,
        在 T5 hidden(512)空间做批量 flow matching 增强。
        ...
        # 真实 denoiser 批量路径(issue #37)
        if (
            self._denoiser is not None
            and self._model_fn_cond is None
            and isinstance(self.encoder, ELFNativeEncoder)
        ):
            return self._enhance_batch_with_denoiser(
                texts, steps, noise_t, cfg_scale, rng, blend_alpha, encode_batch_size
            )
        ...
        logger.debug(
            "enhance_batch(fallback): n=%d, steps=%d, noise_t=%.2f, cfg_scale=%.1f",
            len(texts),
            steps,
            noise_t,
            cfg_scale,
    ...
    # ── 批量 denoiser 增强(issue #37) ──────────

    def _enhance_batch_with_denoiser(
        self,
        texts: list[str],
        steps: int,
        noise_t: float,
        ...
        """在 T5 hidden 空间批量做 flow matching 增强, 与原始编码混合后投影回 768。"""
        if self._denoiser is None:
            raise RuntimeError("denoiser 不可用")
        if not isinstance(self.encoder, ELFNativeEncoder):
            raise RuntimeError("批量 denoiser 增强需要 ELFNativeEncoder")

        n = len(texts)
        # 原始编码用于 blend
        raw = self.encoder.encode_batch(texts, batch_size=encode_batch_size)  # (N, 768)

        # T5 mean-pooled hidden
        pooled = self.encoder._pooled_torch(texts, batch_size=encode_batch_size)  # (N, 512)
        z0 = pooled.astype(np.float32) / LATENT_STD

        if rng is None:
            rng = np.random.default_rng()
        eps = rng.standard_normal((n, T5_HIDDEN_DIM), dtype=np.float32) * DENOISER_NOISE_SCALE
        z_t = ((1.0 - noise_t) * z0 + noise_t * eps).astype(np.float32)

        # ODE 去噪: t 从 noise_t 线性降至 t_eps
        t_vals = np.linspace(noise_t, T_EPS, steps + 1)
        x_pred_prev: NDArray[np.float32] | None = None
        for i in range(steps):
            v, x = self._denoiser_step(z_t, float(t_vals[i]), x_pred_prev, cfg_scale)
        ...
        pooled_out = z0_pred.astype(np.float32) * LATENT_STD
        denoised = self.encoder.embed_from_pooled(pooled_out)  # (N, 768)
        ...
        logger.debug(
            "enhance_batch(denoiser): n=%d, steps=%d, noise_t=%.2f, cfg_scale=%.1f, "
            "blend_alpha=%.2f",
            n,
            steps,
            noise_t,
